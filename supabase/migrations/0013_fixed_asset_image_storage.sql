create table fixed_asset_images (
  id         uuid primary key,
...
alter table fixed_asset_images enable row level security;

create policy fixed_asset_images_select on fixed_asset_images
  for select to authenticated using (
    exists (
      select 1 from fixed_assets a
      where a.id = fixed_asset_images.asset_id
        and a.shop_id = my_shop_id()
    )
  );

create policy fixed_asset_images_owner_insert on fixed_asset_images
...
create policy fixed_asset_images_owner_update on fixed_asset_images
...
create policy fixed_asset_images_owner_delete on fixed_asset_images
...
grant select, insert, update, delete on fixed_asset_images to authenticated;

drop trigger if exists set_synced_at_on_update on fixed_asset_images;
create trigger set_synced_at_on_update before update on fixed_asset_images
...
create or replace function fixed_asset_image_object_asset_id(object_name text)
returns uuid
language sql
immutable
set search_path = ''
...
create policy fixed_asset_images_storage_select
on storage.objects for select to authenticated
using (
  bucket_id = 'fixed-asset-images'
  and exists (
    select 1 from public.fixed_assets a
    where a.id = public.fixed_asset_image_object_asset_id(name)
      and a.shop_id = public.my_shop_id()
  )
);

create policy fixed_asset_images_storage_insert
...
create policy fixed_asset_images_storage_update
...
create policy fixed_asset_images_storage_delete
