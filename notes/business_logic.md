...
## ৮. বিল্ড-টাইম রিফাইনমেন্ট (এই ডকুমেন্ট লেখার পর চূড়ান্ত হওয়া সিদ্ধান্ত)

উপরে যা লেখা আছে তার সব বাস্তবায়িত হয়েছে (M0–M4, v1 ডিলিটসহ)। এখানে
তালিকাভুক্ত করা হলো বানানোর সময় যেসব প্রশ্নের উত্তর ঠিক করা হয়েছে —
ASK-এ না থেকে নিশ্চিতভাবে সিদ্ধান্ত নিয়ে বিল্ড করা হয়েছে:

### ৮.১ ট্রিপ-লেভেল খরচের fund-source অ্যাট্রিবিউশন (§ক-এর খোলা প্রশ্ন)

স্পেকে প্রতিটা PurchaseItem-এর আলাদা fund source আছে, কিন্তু
`transportCost`/`otherCosts`/`cashReturned` — এগুলো ট্রিপ-লেভেল, প্রতি
আইটেমের না — কোন ফান্ড থেকে গুনতে হবে তা বলা নেই। **সিদ্ধান্ত: ট্রিপ-লেভেল
খরচ পুরোপুরি দোকানের নিজের (shop) fund source-এ চার্জ হয়** — ইনভেস্টরের
এন্ট্রিতে শুধু তার কেনা আইটেমের টাকা, দোকানের এন্ট্রিতে নিজের আইটেম +
সম্পূর্ণ ট্রিপ-ওভারহেড। `byFundSource` তাই সবসময় `totalCashOut`-এর সাথে
মিলে যায় (`purchase_reconciliation.dart`-এর ফাইল-লেভেল ডক-কমেন্টে এই
সিদ্ধান্ত সরাসরি লেখা)। ভবিষ্যতে কোনো ইনভেস্টর পুরো ট্রিপ-খরচ বহন করলে
সেটা আলাদা পণ্য-সিদ্ধান্ত দরকার — ফাংশন নিজে অনুমান করবে না।

### ৮.২ পারমিশন মডেল সহজীকরণ

প্রস্তাবনার সময়ের জটিল ক্যাপাবিলিটি-ম্যাট্রিক্স ভাবনার জায়গায় **দুটো
রোলের একটা ফ্ল্যাট নিয়ম**ই চূড়ান্ত হয়েছে: `owner` = নিজের দোকানের
সবকিছুতে SELECT/INSERT/UPDATE/DELETE, `staff` = শুধু SELECT (view-only)।
কোনো টেবিলে ব্যতিক্রম নেই — Postgres RLS-এ `apply_standard_rls` /
`apply_append_only_rls` দুইটা একই নিয়ম সব টেবিলে প্রযোজ্য, আর staff
ডিভাইসের সিঙ্ক-পুশও সেই RLS-ই রিজেক্ট করে (`42501` — লাইভ যাচাই করা
হয়েছে)।

### ৮.৩ Quick Capture-এর বাস্তব রূপ (M3)

§৩-এর QuickCapture ধরন `voiceNote`/`photoNote` দুইটাই UI-তে আছে, কিন্তু
**আসল ভয়েস/ছবি রেকর্ড এখনো নেই** — বর্তমানে `fileLocalPath`-এ একটা
ফ্রি-টেক্সট নোট সংরক্ষিত হয় ("native capture নেই বলে" — কন্ট্রোলারের
ডক-কমেন্টে এটা স্পষ্ট লেখা)। পেন্ডিং-ক্যাপচারকে আসল Sale/Purchase/Expense-এ
রূপান্তরের ফ্লো সম্পূর্ণ কাজ করে। Media capture ও `SyncPendingUploads`
আপলোড — ভবিষ্যৎ কাজ হিসেবে ফ্ল্যাগ করা আছে।

### ৮.৪ রিমাইন্ডারের সম্পূর্ণ তালিকা (M4)

§ছ-এ বাকি/খাতার reminder-এর কথা ছিল — চূড়ান্ত ইঞ্জিনে মোট **৬ ধরনের**
রিমাইন্ডার আছে (`reminder_engine.dart`):
১. Due balance (বাকি), ২. Investor capital return (মূলধন ফেরতের দিন),
৩. Investor profit payout (লাভ-ভাগের চক্র), ৪. Suspicious customer
(ফলোআপ দরকার), ৫. Overdue rent (ভাড়া ফেরতের তারিখ পেরিয়ে গেছে),
৬. **Order deadline** (অর্ডারের `neededByDate`) — এটা M3-তে "কাজ হয়ে
গেছে" বলা হলেও আসলে কানেক্ট ছিল না; M4-তে (PR #26) পুরোভাবে যুক্ত হয়েছে।

### ৮.৫ অডিট লগ, রিসাইকেল বিন ও রিটেনশন (M4)

সব মিউটেশন-যেমন ডিলিট/রিস্টোর-অডিট-লগে লেখা হয়; সফট-ডিলিট করা রো
রিসাইকেল বিনে দেখা যায়; রিটেনশন-উইন্ডোর বাইরে গেলে `RetentionPolicyUseCase`
পরিষ্কার করে — কিন্তু কোনো কাস্টমারের সাথে Dues/Orders/RentTransactions/
Sales-এর FK-ইতিহাস থাকলে তাকে হাইলি **ক্র্যাশ না করে বাদ দেওয়া হয়**
(সফট-ডিলিট অবস্থায় থাকবে) — PR #25-এ এই ক্র্যাশ-ঝুঁকি ধরা ও ঠিক করা
হয়েছে।

### ৮.৬ বাকি যা যা সিদ্ধান্ত-বদল

- **বাইরের সবকিছু স্পেক অনুযায়ীই আছে** — মুলত §৬-এর Legacy Settlement
  ফ্লো ও প্রাইসিং-ইঞ্জিন অটো-বুটস্ট্র্যাপ Y বাস্তবায়িত (বুটস্ট্র্যাপের
  টাইমিং-বাগ PR #19-এ ধরা/ঠিক হয়েছে)।
- পেমেন্ট-মেথডের তিনটা সাব-ব্যালেন্স (নগদ/মোবাইল ব্যাংকিং/ব্যাংক) §৩
  অনুযায়ী ড্যাশবোর্ড "Total Cash" কার্ডে আছে।
- ভাড়া-রাউটারের "বাকি" ফ্লো-তে Sale-সহ Due অটো-তৈরি হয় — §গ অনুযায়ী।
